...
Future<void> shareWorkoutPdf(WorkoutPlan plan, {String? userName}) async {
  ...
  content.add(pw.Text(plan.name,
      style: pw.TextStyle(
          fontSize: 20, fontWeight: pw.FontWeight.bold, color: _navy)));
  if (userName != null && userName.isNotEmpty) {
    content.add(pw.Text(userName,
        style: const pw.TextStyle(fontSize: 11, color: _gray)));
  }
  content.add(pw.Text('Data: ____/____/________',
  ...
  if ((plan.notes ?? '').trim().isNotEmpty) {
    content.add(pw.Text(plan.notes!.trim(),
        style: pw.TextStyle(
            fontSize: 10, color: _gray, fontStyle: pw.FontStyle.italic)));
  }
    ...
    for (final block in _group(exercises)) {
      if (block.length > 1) {
        ...
        content.add(pw.Padding(
          ...
          child: pw.Text(isCircuit ? 'CIRCUITO' : 'SUPER SERIE',
              style: pw.TextStyle(
                  fontSize: 9,
  ...
  doc.addPage(pw.MultiPage(
    ...
    footer: (ctx) => pw.Container(
      ...
      child: pw.Text('PalestrIA   ·   ${ctx.pageNumber} / ${ctx.pagesCount}',
