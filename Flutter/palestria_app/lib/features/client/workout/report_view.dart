...
class _ReportViewState extends ConsumerState<ReportView> {
  ...
  Future<List<Map<String, dynamic>>> _load() async {
    ...
    final rows = await ref
        ...
        .select(
            'id, year_month, tone, narrative, generated_at, status')
        .eq('user_id', uid)
        .eq('status', 'generated')
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          if (ref.read(sessionProvider) == null) {
            return const Center(
                child: Text('Devi essere loggato per vedere i report.',
                    style: AppText.meta));
          }
          final reports = snap.data ?? const [];
          final available = _availableMonth;
          final monthReports =
              reports.where((r) => r['year_month'] == available).toList();
          final tonesGenerated =
              monthReports.map((r) => r['tone'] as String?).toSet();
                ...
                Row(
                  children: [
                    for (final t in _tones) ...[
                      ...
                      if (t != _tones.last) const SizedBox(width: AppSpacing.sm),
                    ],
                  ],
                ),
                ...
                if (reports.isEmpty)
                  ...
                  for (final ym in months) ...[
                    Padding(
                      ...
                      child: Text(_formatYearMonth(ym),
                          style: const TextStyle(
                              fontSize: 12.5,
                              fontWeight: FontWeight.w700,
                              color: AppColors.muted)),
                    ),
                    for (final r in (byMonth[ym]!
                      ..sort((a, b) => (b['generated_at'] as String? ?? '')
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(_formatYearMonth(availableMonth),
              style: const TextStyle(
                  fontSize: 22,
                  fontWeight: FontWeight.w800,
                  color: Colors.white)),
              ...
              Expanded(
                child: ClipRRect(
                  ...
                  child: LinearProgressIndicator(
                    ...
                    valueColor:
                        const AlwaysStoppedAnimation(Color(0xFFC4B5FD)),
                  ),
                ),
              ),
              const SizedBox(width: 8),
              Text('$remaining / $_maxGenerations',
                  style: const TextStyle(
                      fontSize: 13,
                      fontWeight: FontWeight.w800,
                      color: Colors.white)),
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Text(done ? '✓ Generato — apri' : t.desc,
                  textAlign: TextAlign.center,
                  maxLines: 2,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(tone?.label ?? (r['tone'] as String? ?? '—'),
                  style: const TextStyle(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Text('Dati analizzati:',
                    style: TextStyle(fontWeight: FontWeight.w700)),
                const Text('• Prenotazioni (sessioni completate, cancellate, aderenza)\n'
        ...
        builder: (ctx, scroll) => Column(
          children: [
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Text(_formatYearMonth(report['year_month'] as String?),
                      style: const TextStyle(
                          fontSize: 16, fontWeight: FontWeight.w800)),
                  const Spacer(),
                  Text('${tone?.icon ?? ''} ${tone?.label ?? ''}',
                      style: const TextStyle(
            ...
            Expanded(
              child: ListView(
                ...
                children:
                    _markdownWidgets((report['narrative'] as String?) ?? ''),
              ),
            ),
          ],
        ),
