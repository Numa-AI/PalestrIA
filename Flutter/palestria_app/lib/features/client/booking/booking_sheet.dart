    ...
    try {
      await client.functions.invoke(
        ...
        body: {
          ...
          'date_display': dateDisplayOf(day),
          'date': slot.date,
          'time': slot.time,
      ...
      // La prenotazione è già confermata: la notifica non deve annullare l'esito.
    }
    if (!mounted) return;
    setState(() {
      _confirmed = Booking(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '📅 ${booking.dateDisplay} · 🕐 ${booking.time}',
...
class _AttendeesList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final attendeesAsync = ref.watch(
      _attendeesProvider((slot.date, slot.time)),
    );
    final config =
      ...
      data: (attendees) {
        if (attendees.isEmpty) {
          return const Padding(
            padding: EdgeInsets.all(12),
            child: Text(
              'Nessuna persona visibile per questo slot.',
