...
- **2026-07-08 (passata grafica)** — **Design system + uniformazione grafica dell'app.** Nel port PWA→Flutter la grafica era divergente (~250 colori hardcoded, 342 `TextStyle` ad-hoc, 0 font custom, card/empty/pill/ombre reinventati per schermata). Fondazione creata: **font unico Inter** bundlato (`assets/fonts`, 5 pesi, cablato via `kFontFamily` in `tokens.dart`/`org_theme.dart`), **`docs/DESIGN_SYSTEM.md`** (reference unico dello stile), **`lib/core/theme/ui_kit.dart`** (AppCard/SectionHeader/StatusPill/AppEmptyState/GradientButton/AppStatCard/DarkHero/AppLoading/AppErrorRetry/AppSnack + brandGradient) e token estesi (superfici soft, verdi, stati documento, `AppGradients.workoutHero`). Poi **9 aree uniformate in parallelo** (7 audit + 9 subagent di fix in 2 ondate, `core/theme` off-limits come contratto, `flutter analyze` pulito per scope): hardcoded→token; elementi branded→`colorScheme` (dock/tab/selezioni org-aware, prima viola fisso); stati loading/empty/error via UI kit (prima un errore appariva come "lista vuota"); SnackBar colorati per esito (prima tutti navy); `StatusPill` per i badge; + micro-fix (logo studio in login/header, back-to-login, logout rosso, off-by-one cert, maxLength CF/CAP, password OAuth, gating cert editabile, badge Assicurazione, importi Registro via `bookingPrice`, data/ora lezione, messaggi maxLength+conferma+validazione, dropdown vincolati fuso/valuta per sicurezza DST, plural+virgola+seleziona-tutto pagamenti, header "Serie", PDF "Fatto ✓", drag handle + guard swipe). `flutter analyze lib` full **pulito**, 50 file toccati, **NON committato/buildato** (lo fa l'utente). **App nativa + bottom nav cliente lasciate** (richiesta utente), reso solo il colore org-aware. Le **funzioni grosse mancanti** (aumento capienza/"posto extra" + editor Override orari [dead-end attuale], flusso "Registra incasso", editor contatto/elimina cliente, picker catalogo 7200/superset/circuiti, backup/health-check, OAuth, reset-password deep link…) **non costruite** in questa passata grafica → elenco **DEFERITE** in `todo.md` da prioritizzare.
