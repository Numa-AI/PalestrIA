...
## 2026-07-14 - Code review completa: dead-code sweep + fix di correttezza

**Problema/feature.** Review multi-agente dell'intero codebase (19 aree, verifica adversariale di ogni rimozione: un simbolo è "morto" solo se non referenziato in JS, HTML inline/onclick, CSS, SQL, edge, Flutter, sw.js). Rimossi ~1000 righe nette di codice morto e applicati i fix di correttezza confermati. Molti finding sono specifici del multi-tenant PalestrIA; qui sotto la parte **portabile** a un gestionale gemello con la stessa base storica.

### Parte tecnica — fix di correttezza portabili

- **Salvataggi cert/assicurazione ottimisti** (`admin-analytics.js`, `saveCertDate`/`saveAssicDate`): chiamavano l'update del profilo server senza `await` e ne ignoravano l'esito `{ok:false}` → toast di successo anche a scrittura fallita (dato solo in cache locale, perso al logout). Fix: funzioni rese `async`, `const res = await _updateSupabaseProfile(...)`; su `!res.ok` toast di errore e `return` senza chiudere il modal.
- **Classe `extra-picker-open` orfana sul `<body>`** (`admin-calendar.js`, `renderAdminDayView`): dopo `bookForClient()` il re-render rimuoveva i modal `.extra-picker` dal DOM ma non la classe dal body (che ha `overflow:hidden` + nasconde dock/FAB) → scroll bloccato permanente. Fix: accanto a `document.querySelectorAll('body > .extra-picker').forEach(p => p.remove())` aggiungere `document.body.classList.remove('extra-picker-open')`.
- **Annullamento locale silenzioso** (`admin-calendar.js`, `_cancelLocal` in `deleteBooking`): se `cancelAndConvertSlot` ritorna `false` la funzione usciva senza alcun toast → l'admin credeva di aver annullato. Fix: toast di errore esplicito nel ramo `!ok`.
- **Chips filtri clienti sparite dopo una ricerca** (`admin-clients.js`): `showSingleClientCard` setta `style.display='none'` inline su `#clientsFilterChips`, ma né `clearClientsSearch` né `renderClientsTab` lo ripristinavano (ripristinavano solo statsGrid e filterToggle) → lo style inline vince per sempre sulla classe `.open`. Fix: ripristinare anche `filterChips.style.display = ''` in entrambe.
- **Ricerca "Importa" con contatori stantii** (`admin-importa.js`, `_importaOnSearch`): il debounce ri-renderizzava solo la griglia, lasciando la bulk bar "Importa tutti N" e il bottone ✕ (che vivono fuori dalla griglia) non aggiornati. Fix: chiamare il re-render completo `_renderImportaUI(container)` come fanno già pick-categoria e clear, poi ripristinare focus/cursore dell'input ricostruito.
- **Stato "premuto" del toggle privacy mai applicato** (`admin.js`): i consumer (sidebar desktop `aria-pressed`, bottom sheet mobile, CSS `.active`) leggono `classList.contains('active')` su `#btnToggleSensitive`, ma nessuno la settava; inoltre `switchTab` la azzerava su tutti gli `.admin-tab`. Fix: `btn.classList.toggle('active', _sensitiveHidden)` in `_applyPrivacyMask()` + esclusione del bottone privacy dal loop di `switchTab`.
- **Filtro Realtime org catturato troppo presto** (`maintenance.js`): `const orgId = window._orgId` era valutato all'eval dello script, prima che l'auth async lo settasse → subscription sempre senza filtro. Fix: leggere `window._orgId` DENTRO la factory del canale (rivalutato a ogni subscribe/reconnect).
- **URL edge function hardcoded su progetto dismesso** (`allenamento-report.js` `REPORT_FN_URL`, `allenamento.html` `_IMG_PROXY`): project-ref Supabase cablato diverso da quello del client → 401 sistematico. Fix: derivare da `SUPABASE_URL` già globale (``${SUPABASE_URL}/functions/v1/<nome>``).
- **Nomi file export brandizzati hardcoded** (`admin-analytics.js` ×2, `admin-registro.js`): prefisso `TB_` (brand di un altro progetto) nei file XLSX. Fix: prefisso derivato dallo slug org (`window._orgSlug` sanificato) con fallback vuoto.
- **`OrgSettings.load()` marcava `_loaded=true` anche su errore** (`org-settings.js`): dopo un blip di rete al boot nessuna `load()` non forzata ritentava il fetch → settings stantii per tutta la sessione. Fix: `_loaded = true` solo nel ramo a fetch riuscito.
- **`clearAllOrgData` senza marker cross-device** (`admin-settings.js`): il percorso UI vivo del clear dati non scriveva il marker `data_cleared_at` (lo scriveva solo una funzione morta rimossa) né puliva le cache locali. Fix: dopo la RPC, upsert del marker + `dataLastCleared` + svuotamento cache + reload.
- **Prefill nuova scheda perso su mobile** (`admin-schede.js`): il prefill nome/cliente ("Actual → + Aggiungi") era applicato solo dall'editor desktop; su mobile andava perso e restava pendente. Fix: spostato nel dispatcher `_renderPlanEditor` DOPO il render (gli id `#schedePlanName`/`#schedeClientSearch` sono identici nei due editor).

### Parte tecnica — 2ª passata: valori demo/gemello eliminati + refactor di qualità

**Valori hardcoded del gemello resi per-org** (scelta utente: se non configurati → nascosti/omessi, MAI mostrare i dati di un altro studio):

- **Inviti calendario senza indirizzo cablato** (`js/booking.js`): nuovo helper `_orgAddressText()` che compone "Via, Città" da OrgSettings `company.address` ({via,citta,paese}); `googleCalendarUrl()` aggiunge `&location=` solo se non vuoto; `downloadIcs()` include la riga `LOCATION:` solo se non vuota, con escaping ICS di `\ ; ,`.
- **Codice accesso studio per-org** (`prenotazioni.html` + `js/admin-settings.js`): nuova chiave OrgSettings **`company.access_code`** (testo libero, es. "4729 + ▲") con campo dedicato in Impostazioni → Dati azienda; in "Le mie prenotazioni" la funzione `_applyAccessCode()` valorizza `#prenoAccessCode` via `textContent` (no innerHTML col valore) e nasconde la riga se vuoto; ri-applicata dopo `OrgSettings.load()` perché al primo load la cache può essere vuota.
- **Link Google Maps della home nascosto di default** (`index.html`, `js/org-settings.js`, `js/branding-boot.js`): l'`<a data-org-maps>` statico perde href e testo demo e nasce con `display:none`; `applyBranding()` lo mostra e setta l'href SOLO se `company.maps_url` è configurato (con validazione http/https già presente), altrimenti lo tiene nascosto; se maps c'è ma l'indirizzo no, lo span `[data-org-address]` riceve l'etichetta "Indicazioni stradali"; `branding-boot.js` (snapshot pre-paint) mostra il link solo se lo snapshot ha `maps`.

**Refactor di qualità applicati** (comportamento invariato, meno copia-incolla e round-trip):

- `admin-analytics.js`: factory **`_makeExpiryModal(cfg)`** genera open/close/save per i modali Certificato/Assicurazione (config: localField/historyField/remoteField/ids DOM/testi/renderBadge; stato nella closure, rimossi i 2 set di globali `_certModal*`/`_assicModal*`); **`_generateFiscalXlsx(from,to,filename)`** condivisa dai due report fiscali (settimanale e completo) + `_orgFilePrefix()`; trend occupazione con **bucketing una-passata** `countsByMonth[b.date.slice(0,7)][slotType]` invece di 3 filter × 13 mesi con `new Date` per booking; loop "giorni programmati/futuri senza slot" calcolato UNA volta e riusato da weeklyAvg/avgRevPerSchedDay/scheduleEstimate (sia nel dettaglio Fatturato sia in quello Prenotazioni).
- `admin-schede.js`: factory **`_schedeMakeClientSearch(inputId,dropdownId,selectFnName,emptyText)`** per le due ricerche-clienti debounced + `_schedeApplyClientSelection()` condivisa dai due handler di selezione; avatar unificati (**`_schedeAvatarHash`** 0-4 + `_schedeAvatarInitials`, mappati alle due convenzioni CSS nei call-site).
- `data.js` + `admin-schede.js`: nuovo **`WorkoutPlanStorage.deleteExercises(ids)`** (una `DELETE .in('id',ids)`) usato da rimozione giorno/super serie/circuito (prima N round-trip seriali); `_schedeUpdateCircuitRounds` con `Promise.all`; **`addCircuit`** calcola `maxOrder` una sola volta e passa `sort_order` espliciti (da 2N a N+1 query).
- `admin-settings.js`: tutti i bottoni "Salva" delle Impostazioni raggruppano le `OrgSettings.set` (chiavi indipendenti) in **`Promise.all`** (company 9, branding 6, locale 5, policy 5, notifiche 5, GDPR 3 — latenza percepita ~1 RPC invece di N); `_settApplyBrandingExtras` ridotta alla sola delega a `OrgSettings.applyBranding()` (il fallback duplicato era irraggiungibile: ordine di caricamento script + cache-busting coerente).

### Parte tecnica — pattern di dead-code sweep (metodo replicabile)

Categorie di codice morto trovate e rimosse (utili come checklist sul gemello): wrapper `@deprecated`/"backward compat" senza più call-site; variabili globali write-only (assegnate, mai lette); funzioni con bottoni/select che non esistono più in nessun HTML; catene UI legacy sostituite da flussi nuovi; residui del sistema crediti/bonus (JS + interi blocchi CSS `.credit-*`/`.debt-threshold-*`/`.rtype-credit` ecc.); chiavi/branch di config per feature di un ALTRO progetto; export di moduli mai consumati; icone SVG in mappe mai lette. Verifica sempre repo-wide (inclusi HTML inline, `onclick`, `window.X`, RPC in SQL, sw.js APP_SHELL) prima di rimuovere. Cache-bust obbligatorio a fine sweep.
